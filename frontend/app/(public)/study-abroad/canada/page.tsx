...
const FACTS = [
  { label: "Avg Tuition", value: "CAD 20,000–50,000" },
            ...
            { label: "Intakes", value: "Fall (Sep), Winter (Jan), Spring (May)" },
            { label: "Universities", value: "60+" },
];

const HIGHLIGHTS = [
  { text: "Post-graduation work permit up to 3 years" },
                { text: "Pathway to permanent residency" },
                { text: "Multicultural & safe environment" },
                { text: "World-class cities" },
];
    ...
    <Box>
      <Box sx={{ background: "linear-gradient(135deg, #1d4ed8 0%, #2563eb 100%)", py: { xs: 10, md: 14 }, px: { xs: 3, md: 6 }, color: "white" }}>
        <Box sx={{ maxWidth: 860, mx: "auto", textAlign: "center" }}>
          ...
          <Typography variant="h3" fontWeight={900} mb={2}>Study in Canada</Typography>
          <Typography sx={{ opacity: 0.85, fontSize: 18, mb: 5 }}>
            Explore 60+ universities with expert counselor support.
          </Typography>
          <Button
            component={Link}
            href="/consultation"
      ...
      <Box sx={{ py: { xs: 8, md: 12 }, px: { xs: 3, md: 6 } }}>
        <Box sx={{ maxWidth: 1100, mx: "auto" }}>
          ...
          <Card elevation={0} sx={{ border: "1px solid", borderColor: "divider", mb: 8 }}>
            <CardContent sx={{ p: 4 }}>
              <Typography variant="h5" fontWeight={700} mb={3}>Why Study in Canada?</Typography>
          ...
          <Typography variant="h5" fontWeight={700} mb={4}>Universities in Canada</Typography>
          <UniversityListClient defaultCountry="canada" />
        </Box>
      </Box>
    </Box>
  );
