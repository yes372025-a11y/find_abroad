...
async def list_universities(
    ...
    rows = (
        await db.execute(query.order_by(University.world_ranking.asc()).offset(offset).limit(per_page))
    ).scalars().all()
...
async def get_university(slug: str, db: Annotated[AsyncSession, Depends(get_db)]) -> UniversityRead:
    ...
    result = await db.execute(
        select(University).where(University.slug == slug, University.deleted_at.is_(None))
    )
