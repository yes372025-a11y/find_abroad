...
async def seed_counselor(session: AsyncSession) -> None:
    """Create the default counselor account if it doesn't already exist.

    Credentials are read from COUNSELOR_EMAIL / COUNSELOR_PASSWORD env vars.
    Skips silently if either var is missing or the account already exists.
    """
    email = settings.counselor_email
    password = settings.counselor_password

    if not email or not password:
        logger.info("counselor_seed_skip", reason="COUNSELOR_EMAIL or COUNSELOR_PASSWORD not set")
        return
    ...
    if existing is not None:
        logger.info("counselor_seed_skip", reason="account already exists", email=email)
        return
    ...
    counselor = User(
        ...
        full_name=settings.counselor_name,
        role="counselor",
        is_active=True,
        is_verified=True,
    )
    session.add(counselor)
    logger.info("counselor_seeded", email=email)
    print(f"   ✓ Counselor account created: {email}")
...
async def seed_programs(session: AsyncSession, force: bool = False) -> int:
    """Seed programs from CSV. Returns count of rows inserted."""
    existing_count = (
        await session.execute(select(func.count()).select_from(Program))
    ).scalar_one()

    if existing_count > 0 and not force:
        logger.info("programs_skip", existing=existing_count)
        return 0

    if force and existing_count > 0:
        await session.execute(text("DELETE FROM programs"))
        logger.info("programs_cleared", deleted=existing_count)

    # Build slug -> id map for universities
    uni_rows = (await session.execute(select(University.slug, University.id))).all()
    slug_to_id: dict[str, uuid.UUID] = {slug: uid for slug, uid in uni_rows}

    csv_path = DATA_DIR / "programs.csv"
    if not csv_path.exists():
        logger.info("programs_skip", reason="programs.csv not found")
        return 0
    ...
    with csv_path.open(newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for i, row in enumerate(reader, start=1):
            uni_slug = row.get("university_slug", "").strip()
            uni_id = slug_to_id.get(uni_slug)
            if not uni_id:
                logger.warning("program_university_not_found", row=i, slug=uni_slug)
                continue
            try:
                rows.append(Program(
                    id=uuid.uuid4(),
                    ...
                    name=row["name"].strip(),
                    slug=row["slug"].strip(),
                    level=row.get("level", "master").strip(),
                    field=_optional_str(row.get("field", "")),
                    duration_months=_optional_int(row.get("duration_months", "")),
                    tuition_usd=_optional_float(row.get("tuition_usd", "")),
                    is_active=True,
                ))
            except Exception as exc:
                logger.error("program_row_parse_error", row=i, error=str(exc))

    session.add_all(rows)
    logger.info("programs_seeded", count=len(rows))
    return len(rows)
...
async def run_seeds(only: str | None = None, force: bool = False) -> None:
    ...
    async with session_factory() as session:
        async with session.begin():
            ...
            if only is None or only == "programs":
                totals["programs"] = await seed_programs(session, force)
...
def main() -> None:
    ...
    parser.add_argument(
        "--only",
        choices=["universities", "scholarships", "lenders", "programs"],
